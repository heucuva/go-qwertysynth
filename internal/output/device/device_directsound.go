...
func (d *dsoundDevice) Play(in <-chan *premix.PremixData) error {
	return d.PlayWithCtx(context.Background(), in)
}

type playbackData struct {
	event windows.Handle
	row   *premix.PremixData
	pos   int
}
...
func (p *playbackBuffer) Add(mix *mixing.Mixer, row *premix.PremixData, pos int, size int, blockAlign int, panmixer mixing.PanMixer, format sampling.Format) (int, error) {
...
func (d *dsoundDevice) PlayWithCtx(ctx context.Context, in <-chan *premix.PremixData) error {
	maxOutstanding := 3
